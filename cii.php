...
<html lang="en">
    ...
    <body>
                    ...
                    <ul class="navbar-nav ms-auto">
                        ...
                        <li class="nav-item dropdown active">
                            ...
                            <ul class="dropdown-menu dropdown-menu-light dropdown-menu-end" aria-labelledby="navbarDropdown">
                                ...
                                <li><a class="dropdown-item" href="#">Consolidated Item Issue</a></li>
            ...
            <div class="container">
                <div class="row row-header">
                    <div class="col-12 col-sm-12">
                        <h1>Consolidated Item Issue</h1>
                        <p>You can entry/edit consolidated issue of items</p>
                    </div>
                </div>
            </div>
        </header>
        ...
        <nav class="navbar navbar-dark navbar-expand-lg d-flex justify-content-between" id="menu">
            ...
            $sub='submit'.$i;
            if($qnt1!=0){
                echo "<td><input type='number' name='qnt' min='1' max=$qnt1 step='1' value='0'></input></td>";
            ...
            if($_SESSION['date']!=''){
                ...
                $cost1=$sheet->getCell('F'.$i)->getValue();
                $c=($qnt*$cost);
                $sheet->setCellValue('F'.$i,$c+$cost1);
                $qnt1=$sheet->getCell('I'.$i)->getValue();
                $sheet->setCellValue('I'.$i,$qnt+$qnt1);
            ...
            if(isset($_POST[$sub1])){
                if($_SESSION['date']!=''){
                    ...
                    if($numrows==0){
                        mysqli_query($con,"INSERT INTO $table ("."date,"."item,"."sales) VALUES ('".$date."','".$item."','".$c."')");
                    }
                    else{
                        while($row=mysqli_fetch_assoc($query)){
                        $sales=$row['sales'];
                        mysqli_query($con,"UPDATE $table SET sales='".$sales+$c."' WHERE date='".$date."' AND item='".$item."'");
                        }
                    }
                }
            }
